        ...
        <div className="theory-editor">
            ...
            <div className="theory-editor-toolbar">
                ...
                <Tooltip title="Вставить таблицу 3x3">
                    <Button size="small" icon={<TableOutlined />} onClick={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()} />
                </Tooltip>
                <Tooltip title="Добавить строку">
                    <Button size="small" icon={<InsertRowBelowOutlined />} onClick={() => editor.chain().focus().addRowAfter().run()} disabled={!editor.can().addRowAfter()} />
                </Tooltip>
                <Tooltip title="Удалить строку">
                    <Button size="small" icon={<DeleteRowOutlined />} onClick={() => editor.chain().focus().deleteRow().run()} disabled={!editor.can().deleteRow()} />
                </Tooltip>
                <Tooltip title="Добавить столбец">
                    <Button size="small" icon={<InsertRowAboveOutlined style={{ transform: 'rotate(90deg)' }} />} onClick={() => editor.chain().focus().addColumnAfter().run()} disabled={!editor.can().addColumnAfter()} />
                </Tooltip>
                <Tooltip title="Удалить столбец">
                    <Button size="small" icon={<DeleteColumnOutlined />} onClick={() => editor.chain().focus().deleteColumn().run()} disabled={!editor.can().deleteColumn()} />
                </Tooltip>
                <Tooltip title="Удалить таблицу">
                    <Button size="small" onClick={() => editor.chain().focus().deleteTable().run()} disabled={!editor.can().deleteTable()} danger>✕</Button>
                </Tooltip>
