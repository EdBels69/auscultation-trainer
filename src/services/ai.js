...
const MODEL = 'qwen/qwen3.5-plus-02-15'; // all AI features
const QUIZ_MODEL = MODEL;
...
export async function generateQuizQuestions({ count = 5, category = 'all', difficulty = 'medium', soundRecords = [], language = 'ru' }) {
    ...
    const systemPrompt = `Ты — эксперт по аускультации и пропедевтике внутренних болезней.

Правила:
1. Вопросы должны быть РАЗНООБРАЗНЫМИ по типу — не только «какой феномен», но и:
   – Механизм возникновения звука
   – Дифференциальная диагностика (отличие от похожих феноменов)
   – Клиническое значение находки (какое заболевание наиболее вероятно)
   – Характеристики звука (фаза цикла, тембр, иррадиация)
   – Тактика при данной находке
2. Клиническая виньетка: пол, возраст, жалобы → аускультативная находка → вопрос.
3. Дистракторы клинически правдоподобные, различаются по аускультативным признакам.
4. Объяснения: 2–4 предложения, ключевой признак + механизм + дифдиагностика.
5. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО: markdown, ASCII-таблицы, псевдографика.
6. Чистый текст. Живой язык преподавателя. Только русский.
7. ЗАПРЕЩЕНЫ тривиальные вопросы типа «к какой категории относится звук» или «кардио/пульмо».`;

    const userPrompt = `Создай ${count} тестовых вопроса по аускультации в области ${categoryLabel}, ${diffLabel}.${soundContext}

ВАЖНО: Вопросы должны быть РАЗНООБРАЗНЫМИ. Чередуй следующие типы:
1. Идентификация феномена: «При аускультации выслушивается [описание]. Какой феномен?»
2. Клиническое значение: «У пациента выслушивается [феномен]. Какое заболевание наиболее вероятно?»
3. Механизм: «Чем обусловлен [феномен] при [состоянии]?»
4. Дифференциальная диагностика: «Чем отличается [феномен A] от [феномен B]?»
5. Характеристики: «В какую фазу цикла выслушивается [феномен]?» / «Куда иррадиирует?»

Каждый вопрос — клиническая виньетка с пациентом.

Требования:
- Виньетка: пол, возраст, жалобы, точка аускультации, описание звука
- 4 варианта ответа (a, b, c, d) — клинически правдоподобные
- Один правильный ответ
- Объяснение (2–4 предложения): ключевой признак, механизм, дифдиагностика
- НЕ используй ASCII-таблицы или псевдографику
...
Верни ТОЛЬКО валидный JSON без markdown-обёртки:
{
  "questions": [
    {
      "id": "q1",
      "question": "Текст клинической виньетки с вопросом",
      "options": {
        "a": "Вариант А",
        "b": "Вариант Б",
        "c": "Вариант В",
      ...
      "correct_answer": "a",
      "explanation": "Клиническое объяснение с дифференциальной диагностикой",
